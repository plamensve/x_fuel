(() => {
    function initSiteShellBase() {
        if (window.__GORIVA_SITE_SHELL_BASE_READY__) return;
        window.__GORIVA_SITE_SHELL_BASE_READY__ = true;
    ...
    // site-shell-base may now be loaded asynchronously. Initialize immediately
    // when DOMContentLoaded has already fired instead of silently missing it.
    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", initSiteShellBase, { once: true })
    } else {
        initSiteShellBase()
    }
})()
