...
(() => {
    ...
    const initHero = () => {
        ...
        style.textContent = `
            ...
            .hero-v2-grid{display:grid;grid-template-columns:minmax(360px,.88fr) minmax(0,1.5fr);gap:30px;align-items:stretch;min-height:620px}
            .hero-v2-visual{position:relative;min-height:620px;overflow:hidden;border-radius:46% 46% 42% 18%/28% 35% 27% 18%;background:#eaf4ff;box-shadow:inset 0 0 0 12px rgba(225,238,255,.85)}
            .hero-v2-model{width:100%;height:100%;object-fit:cover;object-position:center 36%;display:block;transform:scale(1.02)}
            .hero-v2-visual::after{content:"";position:absolute;inset:0;border-radius:inherit;border:10px solid rgba(219,234,254,.72);pointer-events:none}
            .hero-v2-visual-fade{position:absolute;inset:0;background:linear-gradient(90deg,transparent 0 82%,rgba(255,255,255,.78) 94%,#fff 100%);pointer-events:none}
