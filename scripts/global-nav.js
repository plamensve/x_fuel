(() => {
    ...
    function ensureTickerScript() {
        ...
        script.src = "/scripts/global-ticker.js?v=20260913-ticker1";
        script.async = false;
        document.head.appendChild(script);
    }
    ...
    function buildHeader() {
        ...
        const existingTicker = document.querySelector("body > .ticker") || document.querySelector(".ticker");
        ...
        header.innerHTML = `<div class="goriva-global-nav-shell"><a class="goriva-global-brand" href="/" aria-label="goriva.online — Начало"><img src="/media/2logo.png" alt="goriva.online logo" width="48" height="48" decoding="async"><span class="goriva-global-brand-copy"><strong>goriva.online</strong><span><i aria-hidden="true"></i>Актуални цени в България</span></span></a><nav class="goriva-global-nav" aria-label="Основна навигация"><button class="goriva-global-menu-toggle" type="button" aria-label="Отвори менюто" aria-expanded="false" aria-controls="goriva-global-menu"><span class="goriva-global-menu-icon" aria-hidden="true"><b></b><b></b><b></b></span></button><div id="goriva-global-menu" class="goriva-global-menu">${renderNavigationItems()}</div></nav><div class="goriva-global-actions" aria-label="Статус на данните"><span class="goriva-data-status" role="status"><span class="goriva-data-status-dot" aria-hidden="true"></span><span>Актуални данни днес</span></span></div></div>`;

        const ticker = existingTicker || document.createElement("div");
        ticker.classList.add("ticker", "goriva-global-ticker");
        ticker.dataset.gorivaGlobalTicker = "true";
        if (!ticker.querySelector("#ticker-content")) {
            ticker.innerHTML = '<div id="ticker-content" class="ticker-content"></div>';
        }
        header.appendChild(ticker);
        header.classList.add("has-goriva-ticker");
    ...
    if(document.body){initGlobalShell();}else if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",initGlobalShell,{once:true});}else{initGlobalShell();}
})();
