(() => {
  ...
  const INSA_LOGO = '/images/station_logos/insa-card-logo.png?v=20260909-1';
  ...
  function buildStations(rows) {
    ...
    const count = $('#insa-station-count');
    if (count) count.textContent = String(state.stations.length);
  }

  function renderSummary(rows) {
    const cards = $('#insa-price-cards');
    const status = $('#insa-price-status');
    ...
    if (state.latestDate && rows.length) {
      ...
      status.textContent = 'В момента няма налични Insa Oil цени за визуализиране.';
    }
  }
  ...
  function renderStations(resetVisible = false) {
    const grid = $('#insa-stations-grid');
    const count = $('#insa-results-count');
    const loadMore = $('#insa-load-more');
    ...
    if (!matches.length) {
      grid.innerHTML = '<div class="eko-empty-state">Няма Insa Oil обекти, които отговарят на избраните критерии.</div>';
  ...
  function bindFilters() {
    const search = $('#insa-search');
    const fuel = $('#insa-fuel-filter');
    const loadMore = $('#insa-load-more');
  ...
  async function init() {
    ...
    try {
      ...
      console.warn('[insa-oil-hub] Failed to load Insa Oil prices', error);
      renderSummary([]);
      const grid = $('#insa-stations-grid');
      if (grid) grid.innerHTML = '<div class="eko-empty-state">Данните за Insa Oil не могат да бъдат заредени в момента.</div>';
      const count = $('#insa-results-count');
